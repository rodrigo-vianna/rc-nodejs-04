...
export class InMemoryAnswerCommentsRepository implements AnswerCommentsRepository {
	...
	public async findById(id: string): Promise<AnswerComment | null> {
		const answerComment = this.items.find(answerComment => answerComment.id.value === id)
		return answerComment ?? null
	}

	public async findManyByAnswerId(answerId: string, { page }: PaginationParams): Promise<AnswerComment[]> {
		return this.items
	...
	public async delete(answerComment: AnswerComment): Promise<void> {
		const index = this.items.findIndex(item => item.id.value === answerComment.id.value)
