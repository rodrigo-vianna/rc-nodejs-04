...
export class InMemoryQuestionCommentsRepository implements QuestionCommentsRepository {
	...
	public async findManyByQuestionId(questionId: string, { page }: PaginationParams): Promise<QuestionComment[]> {
		return this.items
	...
	public async delete(questionComment: QuestionComment): Promise<void> {
		const index = this.items.findIndex(item => item.id.value === questionComment.id.value)
